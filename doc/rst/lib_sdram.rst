...
The XMOS SDRAM library is designed for read and write access to SDRAM device of arbitrary length 32b long word buffers at up to 62.5MHz clock rates. 
...
``lib_sdram`` is intended to be used with the `XCommon CMake <https://www.xmos.com/file/xcommon-cmake-documentation/?version=latest>`_
, the `XMOS` application build and dependency management system.

To use this library in an application include ``lib_sdram`` in the application's ``APP_DEPENDENT_MODULES`` list in
...
All ``lib_sdram`` functions can be accessed via the ``sdram.h`` header file, for example:
...
Overview
========
The example uses the XMOS SDRAM library to accesses memory on the SDRAM. The purpose of this application
is to show how data is written to and read from the SDRAM in a safe manner. 

The example is running on the SLICEKIT-XC200-X1 with XA-SK-SDRAM
...
The ``lib_sdram`` software ZIP package should be downloaded and extracted to a chosen working
directory.
...
.. code-block:: bash
    ...
    cd lib_sdram/examples/app_sdram_demo
...
``app_sdram/bin`` directory — one for each supported build configuration.
...
From an XTC command prompt, the following command should be run from the ``examples/app_sdram_demo``
directory:

.. code-block:: bash

    xrun ./bin/app_sdram_demo.xe
...
All ``lib_sdram`` functions can be accessed via the ``sdram.h`` header file:

.. code-block:: C

    #include "sdram.h"

You will also have to add ``lib_sdram`` to the application's APP_DEPENDENT_MODULES list in CMakeLists.txt, for example:
